...
struct transformer
{
    ...
    transformer(proj_transform const& prj_trans,
               ...
               std::int64_t & y1)
    {
        ...
        {
            std::clog << "failed on " << x << " " << y << "\n";
            return false;
        }
        tr_.forward(&x,&y);
        x = x * scaling_;
        y = y * scaling_;
        x1 = static_cast<std::int64_t>(std::round(x));
        y1 = static_cast<std::int64_t>(std::round(y));
        return true;
    }
...
struct transform_visitor {
    ...
    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::point<double> const& geom)
    {
    ...
    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::multi_point<double> const& geom)
    {
        ...
        for (auto const& pt : geom)
        {
            ...
            if (tr_.apply(pt,pt2))
            {
                new_geom.emplace_back(std::move(pt2));
            }
        }
        return new_geom;
    }

    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::line_string<double> const& geom)
    {
    ...
    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::multi_line_string<double> const& geom)
    {
        ...
        for (auto const& line : geom)
        {
            ...
            for (auto const& pt : line)
            {
                ...
                if (tr_.apply(pt,pt2))
                {
                    new_line.emplace_back(std::move(pt2));
                }
            }
            if (!new_line.empty())
            {
                new_geom.emplace_back(std::move(new_line));
            }
        }
        return new_geom;
    }

    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::polygon<double> const& geom)
    {
        ...
        for (auto const& pt : geom.exterior_ring)
        {
            ...
            if (tr_.apply(pt,pt2))
            {
                new_geom.exterior_ring.emplace_back(std::move(pt2));
            }
        }
        for (auto const& ring : geom.interior_rings)
        {
            ...
            for (auto const& pt : ring)
            {
                ...
                if (tr_.apply(pt,pt2))
                {
                    new_ring.emplace_back(std::move(pt2));
                }
            }
            new_geom.interior_rings.emplace_back(std::move(new_ring));
        }
        return new_geom;
    }

    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::multi_polygon<double> const& geom)
    {
        ...
        for (auto const& poly : geom)
        {
            ...
            for (auto const& pt : poly.exterior_ring)
            {
                ...
                if (tr_.apply(pt,pt2))
                {
                    new_poly.exterior_ring.emplace_back(std::move(pt2));
                }
            }
            for (auto const& ring : poly.interior_rings)
            {
                ...
                new_poly.interior_rings.emplace_back(std::move(new_ring));
            }
            new_geom.emplace_back(std::move(new_poly));
        }
        return new_geom;
    }

    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::geometry_collection<double> const& geom)
    {
        mapnik::geometry::geometry_collection<std::int64_t> new_geom;
        new_geom.reserve(geom.size());
        for (auto const& g : geom)
        {
            new_geom.emplace_back(std::move(mapnik::util::apply_visitor((*this), g)));
        }
        return new_geom;
     }

    mapnik::geometry::geometry<std::int64_t> operator() (mapnik::geometry::geometry_empty const& geom)
    {
        return geom;
    }
...
{
    transformer vs(prj_trans, t_, backend_.get_path_multiplier());
    vector_tile_strategy vs2(prj_trans, t_, backend_.get_path_multiplier());
    std::int64_t minx,miny,maxx,maxy;
    vs.apply(buffered_query_ext.minx(), buffered_query_ext.miny(), minx, miny);
    vs.apply(buffered_query_ext.maxx(), buffered_query_ext.maxy(), maxx, maxy);
    box2d<int> bbox(minx,miny,maxx,maxy);
    transform_visitor transit(vs2);
